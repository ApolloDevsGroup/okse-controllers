        ...
        return new Promise((resolve, reject) => {
            ...
            this.hub.once(`${messageId}:finished`, (data) => {
                switch (data.status) {
                    ...
                    case 'rejected':
                        return reject(new Error('Typed Message Signature: User denied message signature.'));
                    case 'errored':
                        return reject(new Error(`Typed Message Signature: ${data.error}`));
                    default:
                        return reject(new Error(`Typed Message Signature: Unknown problem: ${JSON.stringify(messageParams)}`));
                }
            });
        });
    }
